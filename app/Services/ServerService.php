...
class ServerService
{
    public function getAvailableVless(User $user, $groupId = null): array
    {
        ...
        foreach ($server as $key => $v) {
            ...
            if (!in_array($groupId, $server[$key]['group_id'])) continue;
    ...
    public function getAvailableVmess(User $user, $groupId = null): array
    {
        $groupId = $groupId === null ? $user->group_id : $groupId;
        $servers = [];
        ...
        foreach ($vmess as $key => $v) {
            ...
            if (!in_array($groupId, $vmess[$key]['group_id'])) continue;
    ...
    public function getAvailableTrojan(User $user, $groupId = null): array
    {
        $groupId = $groupId === null ? $user->group_id : $groupId;
        $servers = [];
        ...
        foreach ($trojan as $key => $v) {
            ...
            if (!in_array($groupId, $trojan[$key]['group_id'])) continue;
    ...
    public function getAvailableTuic(User $user, $groupId = null)
    {
        $groupId = $groupId === null ? $user->group_id : $groupId;
        $availableServers = [];
        ...
        foreach ($servers as $key => $v) {
            ...
            if (!in_array($groupId, $v['group_id'])) continue;
    ...
    public function getAvailableHysteria(User $user, $groupId = null)
    {
        $groupId = $groupId === null ? $user->group_id : $groupId;
        $availableServers = [];
    ...
    public function getAvailableShadowsocks(User $user, $groupId = null)
    {
        $groupId = $groupId === null ? $user->group_id : $groupId;
        $servers = [];
    ...
    public function getAvailableAnyTLS(User $user, $groupId = null)
    {
        $groupId = $groupId === null ? $user->group_id : $groupId;
        $servers = [];
    ...
    public function getAvailableV2node(User $user, $groupId = null)
    {
        $groupId = $groupId === null ? $user->group_id : $groupId;
        $servers = [];
    ...
    public function getAvailableServers(User $user)
    {
        ...
        // 避免在每个协议里重复查询父账号（需求书第十三节）。
        $entitlement = $this->resolveEntitlement($user);
        $groupId = $entitlement->getEffectiveGroupId();

        $servers = array_merge(
            $this->getAvailableShadowsocks($user, $groupId),
            $this->getAvailableVmess($user, $groupId),
            $this->getAvailableTrojan($user, $groupId),
            $this->getAvailableTuic($user, $groupId),
            $this->getAvailableHysteria($user, $groupId),
            $this->getAvailableVless($user, $groupId),
            $this->getAvailableAnyTLS($user, $groupId),
            $this->getAvailableV2node($user, $groupId)
        );
